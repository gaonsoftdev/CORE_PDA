...
var app = new Vue({
	...
	methods:{
		...
		openMoveList: function(){
			var vThis = this;
			let params = {QryType:'MoveNoQuery',Gubun:'IDD_MOVE_IN'};
				...
				function(data){
					if(data != null){
						for(let i in data){
							...
							data[i].MovQty = GX.NumberType.quantity(data[i].InQty);
						}
		...
		BarcodeSel: function(){
			...
			bk1: if(vThis.queryForm.MoveNo != '' && vThis.queryForm.BarCode != ''){
				let params = {QryType:'BarCodeQuery',Gubun:'IDD_MOVE_IN',InOutSerl:vThis.rows.MoveData[0].InOutSerl};
		...
		save2: function(){
			...
			if(vThis.QtyData.Qty == vThis.QtyData.MovQty){
				...
				let params = {QryType:'NewSave',Gubun:'IDD_MOVE_IN',SendType:'2'};
