...
var app = new Vue({
	...
	methods:{
		...
		WHSel:function(){
			var vThis = this;
			bk1: if(vThis.queryForm.WHName != ''){
				let params = {
					UCompanySeq: GX.Cookie.get('UCompanySeq'),
					WHName: vThis.queryForm.WHName,
				};
				this.init();
				GX._METHODS_
			...
			if(clipboardData != null){
				...
				if(objAll.length > 0){
					//if(QRCodeData.includes('P') || QRCodeData.includes('H')){
						activeObjName = objAll[0].name;
						activeTabIdx = objAll[0].tabIndex;
					//}
				}
				...
				if(activeObjName.length > 0){
					...
					if (objAll[0] == document.activeElement) {
					   vThis.queryForm.WHName = QRCodeData.replaceAll(' ','');
					   //vThis.WHSel('OutWH');
					   vThis.WHSel();
					//    document.querySelector('[name="ItemNo"]').focus();
		...
		okCodeHelp: function (targetName) { 
			...
			if (targetName == 'Storage') {
				this.queryForm.WHName = this.rows[targetName + 'ListQuery'][obj.selectedIndex].WHName;
	...
	mounted(){
		...
		document.querySelector('[name="WHName"]').focus();
	},
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			// .item('ItemNo').head('품번', '')
			// .item('MngQty').head('장부수량', '')
			// .item('RealQty').head('실사수량', '')
