...
var app = new Vue({
	...
	data:{
		...
		queryForm:{
			Date: GX.getParameters().Date,
			OutWHSeq: GX.getParameters().OutWHSeq,
			ReqQty: GX.getParameters().ReqQty,
			BottleSerl: GX.getParameters().ReqQty,
			BottleSeq: GX.getParameters().BottleSeq,
			Qty: 0,
		},
		QtyData:{
			ItemName:'',
	...
	methods:{
		add: function(){
			...
			if(event.keyCode == 13){
				...
				GX._METHODS_
				.setMethodId('Genuine.enModuleName.BisBottleMovePDA_en/MoveCodeHelpBottle')
				.ajax([params], [function(data){
					...
					if(data[0] != null && data.length == 1){
						...
						if(isDuplicated) alert('동일한 용기가 선택되었습니다');
						else {
							params = {'BottleSeq': BottleSeq};
							GX._METHODS_
							...
							.ajax([params], [function(data){
								...
								if(data[0] != null){
									if(data[0].Status == 0) {
										// vThis.queryForm.Qty = vThis.queryForm.Qty + Number(temp.Qty);
										vThis.queryForm.Qty = vThis.queryForm.Qty + Number(1);
				...
				if(GX.Validation.run('', 'validate')){
					...
					for(let i in BottleList){
						if(BottleList.hasOwnProperty(i)){
							param = {};
							param.Date = vThis.queryForm.Date.replaceAll('-','');
							param.OutWHSeq = vThis.queryForm.OutWHSeq;
							param.BottleSerl = vThis.queryForm.BottleSerl;
							param.BottleSeq = vThis.queryForm.BottleSeq;
					...
					GX._METHODS_
					.setMethodId('Genuine.enModuleName.BisWSLBottleSellAPI_en/SellSave')
					.ajax(params, [function(data){
						console.log(data);
