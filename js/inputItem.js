...
var app = new Vue({
	...
	methods:{
		...
		search: function(){
			...
			// ProcSeq: 0,
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisMatInPutPDA_en/MatInPutQuery2_1X')
			.ajax([params], [function(data){
				console.log(data);
				if(data[0] != null){
					vThis.rows.InputItemList = data;
				}
			}], true);
		...
		checkProductInput: function(){
			...
			console.log(this.isProductInputs);
			//this.isProductInputs = (this.isProductInputs.indexOf(1) != -1) ? [0] : [1];
		},

		//packing화면 이동
		goInputMaterialInfo: function (idx) {
			//console.log(idx, this.rows.InputItemList[idx]);
			let paramData = {};
			paramData.StdUnitQty = this.rows.InputItemList[idx].StdUnitQty;
			paramData.ItemSeq = this.rows.InputItemList[idx].ItemSeq;
			paramData.WorkOrderSeq = this.rows.InputItemList[idx].WorkOrderSeq;
			paramData.WorkOrderSerl = this.rows.InputItemList[idx].WorkOrderSerl;
			location.href = 'inputMaterialInfo.html' + GX.makeQueryString(paramData);
		},
		selectAll: function(){
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('ItemName').head('품명', 'word-break')
			.item('SerialNo',{rowspan:2}).head('선택', 'text-c')
			.body('<button class="btn-style-03" @click="goInputMaterialInfo(index);">선택</button>', 'text-c')
			.nl()
			.item('LotNo').head('LotNo', '')
			.item('StdUnitQty',{rowsapn:2}).head('기준수량', 'border-r text-r')
			.loadTemplate('#grid', 'rows.InputItemList');
