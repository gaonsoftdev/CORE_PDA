...
var app = new Vue({
	...
	methods:{
		//출고증조회
		openMoveList: function(){
			var vThis = this;
			let params = {QryType:'InvNoQuery',Gubun:'IDD_INV_OUT'};
			let inputData = GX.deepCopy(GX.makeParamByForm(['InvoiceNo'])); //참조없는 복사
				...
				function(data){
					if(data != null){
						for(let i in data){
							...
							data[i].Qty = GX.NumberType.quantity(data[i].Qty);
							data[i].InvQty = GX.NumberType.quantity(data[i].OutQty);
						}
						vThis.queryForm.CarNo = data[0].CarNo;
						vThis.queryForm.CustName = data[0].CustName;
		...
		BarcodeSel: function(){
			var vThis = this;
			bk1: if(vThis.queryForm.InvoiceNo != '' && vThis.queryForm.BarCode != ''){
				let params = {QryType:'BarCodeQuery',Gubun:'IDD_INV_OUT'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['InvoiceNo','BarCode'])); //참조없는 복사
				Object.assign(params, inputData);
				for(let i in vThis.rows.Query){
					if(vThis.rows.Query[i].PalletNo.replaceAll(' ','') == params.BarCode.replaceAll(' ','')){
						alert('이미 스캔된 바코드입니다.');
						break bk1;
					}
				}
				...
				.ajax([params],[
					function(data){
						if(data != null){
							for(let i in vThis.rows.MoveData){
								if(vThis.rows.MoveData[i].ItemSeq == data[0].ItemSeq && vThis.rows.MoveData[i].Qty > vThis.rows.MoveData[i].InvQty){
									vThis.rows.MoveData[i].InvQty = Number(vThis.rows.MoveData[i].InvQty) + Number(data[0].InvQty);
									vThis.QtyData.ItemName = vThis.rows.MoveData[i].ItemName;
									vThis.QtyData.Qty = vThis.rows.MoveData[i].Qty;
									vThis.QtyData.InvQty = vThis.rows.MoveData[i].InvQty;
									
									data[0].Qty = GX.NumberType.quantity(data[0].Qty);
									vThis.rows.Query.splice(0, 0, data[0]);
									vThis.initSelected();
								}else if(vThis.rows.MoveData[i].ItemSeq == data[0].ItemSeq){
									alert('출고수량보다 스캔수량이 더 많을수 없습니다.');
									break;
								}
							}//for end
							if(data[0].PalletMessage != ''){
								alert(data[0].PalletMessage);
							}
		...
		save2: function(){
			var vThis = this;
			if(vThis.QtyData.Qty == vThis.QtyData.InvQty){
			// if(1==1){
				let button = event.target;
				let params = {QryType:'NewSave',Gubun:'IDD_INV_OUT',SendType:'1'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['MovementDate','InvoiceNo'])); //참조없는 복사
		...
		scanBarCode: function(){
			...
			if(clipboardData != null){
				...
				if(activeObjName.length > 0){
					...
					if(isChange =='InvoiceNo'){
						document.querySelector('[name="InvoiceNo"]').value = QRCodeData.replaceAll(' ','');
						vThis.openMoveList();
	...
	mounted(){
		...
		this.QtyData.Qty = '0';
		this.QtyData.InvQty = '0';
		document.querySelector('[name="InvoiceNo"]').focus();

		if(GX.Storage.get('InvoiceNo') != null && GX.Storage.get('InvoiceNo') != ''){
			this.queryForm.InvoiceNo = GX.Storage.get('InvoiceNo');
			document.querySelector('[name="InvoiceNo"]').value = GX.Storage.get('InvoiceNo');
			vThis.openMoveList();
		}
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('OverLap').head('재출고여부', '')
			.body('<div class="chkBox" style="text-align:center"><input type="checkbox" name="OverLap" v-model="row.OverLap" true-value="1" false-value="0" disabled /></div>', '')
...
function responseAppInvoiceNo(action, result){
	if(action == 'QRCodeScanner'){ // scanner 관련
		if(typeof result == 'object' && result.param == 'once') {
			app.queryForm.InvoiceNo = result.result;
