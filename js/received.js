...
var app = new Vue({
	...
	data:{
		...
		queryForm:{
			...
			WHSeq:'',
			BottleProcSeq: ((GX.getParameters().BottleProcSeq != null) ? GX.getParameters().BottleProcSeq : 231988002)
		},
		codeHelp:{
			ItemNo:''
		},
	...
	methods:{
		...
		selectWHNameList: function(){
			var vThis = this;
			
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisWPDBottleProcAPI_en/CodeHelpWHSeq')
			.ajax([{}], [function(data){
				console.log(data);
				if(data[0] != null){
					vThis.rows.WHNameList = data;
				}
			}], true);
		...
		add: function(){
			...
			if(event.keyCode == 13){
				...
				GX._METHODS_
				.setMethodId('Genuine.enModuleName.BisWPDBottleProcAPI_en/CodeHelpBottleNo')
		...
		save: function(){
			...
			if(BottleList.length > 0){
				if(GX.Validation.run('', 'validate')){
					...
					const ProcDate = GX.formatDate(this.queryForm.ProcDate, 'YMD');
					...
					for(let i in BottleList){
						if(BottleList.hasOwnProperty(i)){
							...
							param.BottleProcSeq = this.queryForm.BottleProcSeq;
							param.ProcSeq = 0;
							param.WHSeq = this.queryForm.WHSeq;
					...
					GX._METHODS_
					.setMethodId('Genuine.enModuleName.BisWPDBottleProcAPI_en/SaveAPI')
					.ajax(params, [function(data){
						console.log(data);
