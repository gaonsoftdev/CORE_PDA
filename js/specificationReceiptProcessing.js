...
var app = new Vue({
	...
	methods:{
		...
		openMoveList: function(){
			var vThis = this;
			let params = {QryType:'InvNoQuery',Gubun:'IDD_INV_IN_ONE'};
				...
				function(data){
					if(data != null){
						for(let i in data){
							...
							data[i].InvQty = GX.NumberType.quantity(data[i].InQty);
						}
		...
		BarcodeSel: function(){
			...
			bk1: if(vThis.queryForm.InvoiceNo != '' && vThis.queryForm.BarCode != ''){
				let params = {QryType:'BarCodeQuery',Gubun:'IDD_INV_IN_ONE'};
		...
		save2: function(){
			...
			if(vThis.QtyData.Qty == vThis.QtyData.InvQty){
				...
				let params = {QryType:'NewSave',Gubun:'IDD_INV_IN_ONE',SendType:'1'};
