...
var app = new Vue({
	...
	methods:{
		...
		openMoveList: function(){
			var vThis = this;
			let params = {QryType:'MoveNoQuery',Gubun:'IDD_MOVE'};
			// let inputData = GX.deepCopy(this.queryForm);
			let inputData = GX.deepCopy(GX.makeParamByForm(['MoveNo'])); //참조없는 복사
			//this.queryForm.MoveNo = document.querySelector('[name="MoveNo"]').value;
				...
				function(data){
					if(data != null){
						for(let i in data){
							...
							data[i].MovQty = GX.NumberType.quantity(data[i].OutQty);
						}
						vThis.queryForm.CarNo = data[0].CarNo;
						vThis.queryForm.InWhName = data[0].InWhName;
		...
		BarcodeSel: function(){
			...
			bk1: if(vThis.queryForm.MoveNo != '' && vThis.queryForm.BarCode != ''){
				let params = {QryType:'BarCodeQuery',Gubun:'IDD_MOVE',InOutSerl:vThis.rows.MoveData[0].InOutSerl};
				// let inputData = GX.deepCopy(this.queryForm);
				let inputData = GX.deepCopy(GX.makeParamByForm(['MoveNo','BarCode'])); //참조없는 복사
				Object.assign(params, inputData);
					...
					function(data){
						if(data != null){
							for(let i in vThis.rows.MoveData){
								if(vThis.rows.MoveData[i].ItemSeq == data[0].ItemSeq && vThis.rows.MoveData[i].Qty > vThis.rows.MoveData[i].MovQty){
									vThis.rows.MoveData[i].MovQty = Number(vThis.rows.MoveData[i].MovQty) + Number(data[0].MovQty);
									vThis.QtyData.ItemName = vThis.rows.MoveData[i].ItemName;
									vThis.QtyData.Qty = vThis.rows.MoveData[i].Qty;
									vThis.QtyData.MovQty = vThis.rows.MoveData[i].MovQty;
		...
		save2: function(){
			var vThis = this;
			if(vThis.QtyData.Qty == vThis.QtyData.MovQty){
				let button = event.target;
				let params = {QryType:'NewSave',Gubun:'IDD_MOVE',SendType:'1'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['MovementDate'])); //참조없는 복사
		...
		scanBarCode: function(){
			...
			if(clipboardData != null){
				...
				if(activeObjName.length > 0){
					...
					// GX.eventTrigger('[name="'+activeObjName+'"]', 'change');//1 또는 2개 있을 때
					if(isChange =='MoveNo'){
						document.querySelector('[name="MoveNo"]').value = QRCodeData.replaceAll(' ','');
						vThis.openMoveList();
	...
	mounted(){
		...
		this.QtyData.Qty = '0';
		this.QtyData.MovQty = '0';
		document.querySelector('[name="MoveNo"]').focus();
