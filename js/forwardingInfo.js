...
var app = new Vue({
				...
				if(GX.Validation.run('', 'validate')){
					...
					for(let i in BottleList){
						if(BottleList.hasOwnProperty(i)){
							...
							param.BottleMoveSeq = this.queryForm.BottleMoveSeq;
							param.OutWHSeq = this.queryForm.OutWHSeq;
							param.InWHSeq = this.queryForm.InWHSeq;
							param.InOutSeq = 0;
							param.InOutSerl = 0;
							param.InOutType = 80;
							param.InOutNo = '';
							param.BizUnit = BizUnit;
							param.ReqBizUnit = ReqBizUnit;
							param.EmpSeq = EmpSeq;
					...
					GX._METHODS_
					.setMethodId('Genuine.enModuleName.BisBottleMovePDA_en/MoveSave')
					.ajax(params, [function(data){
						console.log(data);
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('ReqSerl',{rowspan:2}).head('삭제', '')
			...
			.item('BottleName').head('용기명', '') //용기명
			// .body('<span>{{row.BottleNo}}</span><input type="text" v-if="index == 0" id="BottleNo" v-model="BottleNo" @keyup="add();" autofocus="autofocus" gx-scanner="Y"> ', 'code-help')
			.item('Qty',{rowspan:2}).head('Qty', '')
			.nl()
			.item('ItemName').head('제품명', '')
			.item('LotNo').head('LotNo', 'border-r').body('{{row.LotNo}}', 'text-r border-r')
