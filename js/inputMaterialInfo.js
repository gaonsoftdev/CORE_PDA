...
var app = new Vue({
	...
	methods:{
		...
		search: function(){
			...
			// ProcSeq: 0,
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisMatInPutPDA_en/MatInPutQuery2_2X')
			.ajax([params], [function(data){
				...
				if(data != null && data[0] != null){
					...
					vThis.rows.InputMaterialInfoList = data;
				}
			}], true);
		...
		sumQty: function(){
			let Qty = 0;
			for(let i in this.rows.InputMaterialInfoList){
				if(this.rows.InputMaterialInfoList.hasOwnProperty(i)){
					Qty += Number(this.rows.InputMaterialInfoList[i].Qty);
				}
			}
		...
		checkFocus: function(index){
			console.log((Number(this.focusRowIndex)==Number(index)), this.focusRowIndex, '=' ,index);
			return Number(this.focusRowIndex)==Number(index);
		},
		focusLotNo: function(index){
			event.target.closest('td').className = '';
			this.focusRowIndex = index;

			console.log((this.focusRowIndex==index), this.focusRowIndex, '=' ,index);
		},
		blurLotNo: function(){
			event.target.closest('td').className = 'code-help';
		...
		save: function(){
			...
			for(let i in this.rows.InputMaterialInfoList){
				if(this.rows.InputMaterialInfoList.hasOwnProperty(i)){
					if(Number(this.rows.InputMaterialInfoList[i].Qty) > 0) params.push(this.rows.InputMaterialInfoList[i]);
				}
			}

			if(params.length > 0){
				GX._METHODS_
				.setMethodId('Genuine.enModuleName.BisMatInPutPDA_en/MatInPutSave2_1X')
				.ajax(params, [function(data){
					console.log(data);

					if(data[0] != null){
						if(data[0].Status == 0) vThis.init(); // 초기화
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('InOutDate').head('입고일자', 'text-c')
			.item('StockQty').head('재고', 'text-r')
			.item('Qty').head('투입', 'text-r')
			.body('<input type="text" id="Qty" v-model="row.Qty" @keyup="sumQty();" @focus="focusLotNo(index);" @blur="blurLotNo();" autofocus="autofocus" gx-scanner="Y">', 'code-help')
			.loadTemplate('#grid', 'rows.InputMaterialInfoList');
