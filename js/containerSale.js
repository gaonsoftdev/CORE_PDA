...
var app = new Vue({
	...
	methods:{
		...
		ReqListSel: function(){
			const vThis = this;
			if (vThis.queryForm.Date != '') {
				vThis.rows.ReqList = [];
				let params = {
					'Date': vThis.queryForm.Date.replaceAll('-',''),
					// 'OutWHSeq': vThis.queryForm.OutWHSeq,
				};
				GX._METHODS_
				.setMethodId('Genuine.enModuleName.BisWSLBottleSellAPI_en/SellQuery')
				.ajax([params], [function(data){
					console.log(data);
					if (data[0] != null) {
						vThis.rows.ReqList = data;
					}
				}], true);
			}
		...
		goPacking: function (idx) {
			if (this.queryForm.OutWHSeq == '') {
				alert('출고창고가 선택되지 않았습니다.');
				return;
			}
			//console.log(idx, this.rows.WorkOrderList[idx]);
			let paramData = this.rows.ReqList[idx];
			Object.assign(paramData, this.queryForm);
			location.href = 'containerInfo.html' + GX.makeQueryString(paramData);
		},
		selectAll: function(){
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('ReqNo',).head('품명', '')
			.item('ReqQty').head('수량', '')
			.item('SerialNo').head('선택', '').body('<button class="btn-style-03" @click="goPacking(index);">선택</button>', 'text-c')
			.loadTemplate('#grid', 'rows.ReqList');
