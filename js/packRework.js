...
var app = new Vue({
	...
	methods:{
		//출고증조회
		ReWorkPalletSel: function(){
			var vThis = this;
			let params = {QryType:'RePalletQuery',Gubun:'IDD_PACK_PALL'};
			let inputData = GX.deepCopy(GX.makeParamByForm(['ReWorkPallet'])); //참조없는 복사
			Object.assign(params, inputData);
			GX._METHODS_
			.setMethodId('ReWorkPal')
			.ajax([params],[
				function(data){
					if(data != null){
						for(let i in data){
		...
		InputPalletSel: function(){
			var vThis = this;
			bk1: if(vThis.queryForm.ReWorkPallet != '' && vThis.queryForm.InputPallet != ''){
				let params = {QryType:'InputPalletQuery',Gubun:'IDD_PACK_PALL'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['ReWorkPallet','InputPallet'])); //참조없는 복사
				Object.assign(params, inputData);
		...
		save2: function(){
			var vThis = this;
			let params = {QryType:'NewSave',Gubun:'IDD_PACK_PALL',SendType:'3'};
			let newData = [];
			...
			if(clipboardData != null){
				...
				if(scannerObj.length > 0){
					if(QRCodeData.includes('R')){
						...
						document.querySelector('[name="ReWorkPallet"]').focus();
						isChange = 'ReWorkPallet';
					} else if(QRCodeData.includes('P') || QRCodeData.includes('H')){
						activeObjName = scannerObj[1].name;
						activeTabIdx = scannerObj[1].tabIndex;
						document.querySelector('[name="InputPallet"]').focus();
				...
				if(activeObjName.length > 0){
					...
					if(isChange =='ReWorkPallet'){
						document.querySelector('[name="ReWorkPallet"]').value = QRCodeData.replaceAll(' ','');
						vThis.ReWorkPalletSel();
					}else if(isChange =='InputPallet'){
						document.querySelector('[name="InputPallet"]').value = QRCodeData.replaceAll(' ','');
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			this.queryForm.Date = GX.formatDate(GX.nowDate().full, 'YMD');
			...
			.item('ReWorkPallet').head('PackPallet', '')
